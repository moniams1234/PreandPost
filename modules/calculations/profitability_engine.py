        ...
        if df_zlec is not None:
            ...
            if nzp_z and wart_z:
                ...
                if rows_for_zlec.any():
                    ...
                    idx_date = matched_sales.index[mask_date_zlec]
                    ...
                    rows_for_zlec_sales = df["Sales Value"].isna()
                    mask_sales = matched_sales["_matched_sales"].notna() & rows_for_zlec_sales
                    idx_sales = matched_sales.index[mask_sales]
                    ...
                    blank_source = df["_matched_source"].fillna("").astype(str).str.strip() == ""
                    common_idx = matched_sales.index.intersection(df.index[blank_source])
