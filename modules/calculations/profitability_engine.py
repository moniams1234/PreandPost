...
def build_profitability(
    ...
    qty_fv_col = fcol(df, "Zamawiana ilość z FV")
    sales_col = fcol(df, "Sales Value")

    ilosc_fv_col = None
    wartosc_fv_col = None

    if df_zf is not None and not df_zf.empty:
        ilosc_fv_col = fcol(df_zf, "Ilosc w linii FV", "Ilość w linii FV")
        wartosc_fv_col = fcol(df_zf, "Wartosc w linii FV netto", "Wartość w linii FV netto")

    if (
        qty_fv_col and sales_col and
        ilosc_fv_col and wartosc_fv_col and
        zp_c and zp_c in df.columns
    ):
        try:
            zf_tmp = df_zf.copy()

            zf_tmp["_zp"] = zf_tmp[zp_c].astype(str).str.strip()
            ...
            def _resolve_sales(row):
                zp = str(row.get(zp_c, "")).strip()

                qty = round(
                    float(
                        pd.to_numeric(
                            row.get(qty_fv_col, 0),
                            errors="coerce"
                        )
                    ),
                    4
