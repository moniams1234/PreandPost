...
def build_profitability(
    ...
    # ── 9b. SALES VALUE MATCH BY FV QTY ──────────────────────────────────────
    # If one production order has multiple invoice rows in zlec+faktury,
    # select Sales Value where:
    #   Zamawiana ilość z FV == Ilość w linii FV.
    # This fixes duplicated invoice lines for the same production order,
    # e.g. 169375/2025.

    qty_fv_col = fcol(df, "Zamawiana ilość z FV", "Zamawiana ilosc z FV")
    if not qty_fv_col:
        # Fallback: in many source files the quantity from FV is stored as "Zamawiana ilość".
        qty_fv_col = fcol(df, "Zamawiana ilość", "Zamawiana ilosc")

    sales_col = fcol(df, "Sales Value")
    df_zlec_src = df_zlec if df_zlec is not None else None

    if df_zlec_src is not None and not df_zlec_src.empty and sales_col:
        nzp_z2 = fcol(df_zlec_src, "Numer zlecenia produkcyjnego")
        ilosc_fv_col = fcol(df_zlec_src, "Ilosc w linii FV", "Ilość w linii FV")
        wartosc_fv_col = fcol(
            df_zlec_src,
            "Wartosc w linii FV netto",
            "Wartość w linii FV netto",
        )

        if qty_fv_col and nzp_z2 and ilosc_fv_col and wartosc_fv_col:
            try:
                zf_tmp = df_zlec_src.copy()

                zf_tmp["_zp"] = zf_tmp[nzp_z2].astype(str).str.strip()
                zf_tmp["_qty"] = pd.to_numeric(
                    zf_tmp[ilosc_fv_col], errors="coerce"
                ).fillna(0)
                zf_tmp["_sales"] = pd.to_numeric(
                    zf_tmp[wartosc_fv_col], errors="coerce"
                ).fillna(0)

                lookup = {}
                for _, r in zf_tmp.iterrows():
                    key = (str(r["_zp"]).strip(), round(float(r["_qty"]), 4))
                    # keep first exact match, similar to XLOOKUP/FILTER first result
                    if key not in lookup:
                        lookup[key] = r["_sales"]

                def _resolve_sales(row):
                    zp = str(row.get("Zlecenie produkcyjne", "")).strip()
                    qty_raw = pd.to_numeric(row.get(qty_fv_col, 0), errors="coerce")
                    try:
                    ...
                    if key in lookup:
                        return lookup[key]
                    return row.get(sales_col, 0)

                df[sales_col] = df.apply(_resolve_sales, axis=1)

            except Exception as exc:
                warns.append(f"Sales Value matching warning: {exc}")

    # ── 9c. CHECK SPRZEDAŻ ────────────────────────────────────────────────────
    # User requested a check column comparing "Zamawiana ilość z FV" with "Sales Value".
    ...
    TAIL = ["Total DL", "Total Materials", "Sales Value",
            "Data faktury", "Miesiąc faktury", "check sprzedaż", "TPM", "TPM%", "CM", "CM%"]
