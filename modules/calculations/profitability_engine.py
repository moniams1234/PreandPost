...
def build_profitability(
    ...
    # Rules:
    # 1) Sales Value can be taken only if production order / invoice line is found
    #    in "zlecenia + faktury" or "faktury linie".
    # 2) If one production order has one invoice row → take its net invoice line value.
    # 3) If one production order has many invoice rows → take the row where
    #    "Zamawiana ilość z FV" equals "Ilosc w linii FV".
    # 4) If still missing, use previous proportional logic from "faktury linie",
    #    but only when a matching invoice line is found:
    #    Sales Value = (Wartosc w linii FV netto / Ilosc w linii FV) * qty.
    # 5) If nothing is found → Sales Value remains 0. No guessing.
    df_zlec = None
        ...
        if df_zlec is not None:
            ...
            if nzp_z and wart_z:
                ...
                def _match_sales_from_zlec(row):
                    ...
                    # One invoice line for production order.
                    if len(g) == 1:
                        r = g.iloc[0]
                        return pd.Series([r["_sales"], r["_date"], "zlec+FV: NZP"])

                    # Multiple invoice lines: match by invoice quantity.
                    ...
                    # Multiple invoice lines but no quantity match: do not sum and do not guess.
                ...
                df.loc[:, "Źródło Sales Value"] = matched_sales["_matched_source"].values

    # ── 5b. faktury linie — proportional fallback only if line is found ───────
    ...
    if uf_fry and missing_mask.any():
        ...
        if df_fry is not None:
            ...
            if nl_c and wl_c:
                fry_nl_lower = [
                    str(s).lower()
                    for s in df_fry[nl_c].fillna("").astype(str).tolist()
                ]
                ...
                for idx in df[missing_mask].index:
                    ...
                    # If no matching invoice line exists, do not assign Sales Value.
                    if not matched:
                        continue

                    # Previous correct behaviour: use first matching invoice line.
                    ...
                    df.at[idx, "Źródło Sales Value"] = source
    ...
    TAIL = ["Total DL", "Total Materials", "Sales Value",
            "Data faktury", "Miesiąc faktury", "Źródło Sales Value", "check sprzedaż", "TPM", "TPM%", "CM", "CM%"]
