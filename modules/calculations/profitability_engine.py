...
def build_profitability(
    ...
    # Sales Value:
    # 1) first: zlecenia + faktury by Numer zlecenia produkcyjnego
    # 2) if many positive rows: match by Ilosc w linii FV
    # 3) add all negative rows for the same production order as corrections
    # 4) fallback to Faktury-linie:
    #    - completely blocked for FD Pharma
    #    - blocked for empty / TEST / non-produced rows
    # 5) no allowed match => Sales Value = 0
    df_zlec = None
    ...
    if uf_zlec:
        ...
        if df_zlec is not None:
            ...
            if nzp_z and wart_z:
                ...
                def _sales_plus_negative(pos_row, group_all):
                    base_sales = float(pos_row["_sales"]) if pd.notna(pos_row["_sales"]) else 0.0
                    neg = group_all.loc[group_all["_sales"].fillna(0) < 0, "_sales"].sum(skipna=True)
                    return base_sales + float(neg)
                ...
                def _match_sales_from_zlec(row):
                    ...
                    if len(positives) == 1:
                        r = positives.iloc[0]
                        return pd.Series([_sales_plus_negative(r, g_valid), r["_date"], "zlec+FV: NZP + korekty ujemne"])
                    ...
                    if qty_fv_base_col:
                        ...
                        if pd.notna(q):
                            ...
                            if len(gm) >= 1:
                                r = gm.iloc[0]
                                return pd.Series([_sales_plus_negative(r, g_valid), r["_date"], "zlec+FV: NZP + ilość FV + korekty ujemne"])
