...
def build_profitability(
    ...
    # Zasada po poprawce:
    # 1) Sales Value może pochodzić wyłącznie z jednoznacznego dopasowania.
    # 2) Dla zlec+faktury sumujemy WSZYSTKIE linie dla numeru zlecenia,
    #    w tym wartości ujemne / korekty z kolumny "Wartosc w linii FV netto".
    # 3) Nie tworzymy / nie dopowiadamy fikcyjnej sprzedaży dla FD Pharma ani
    #    innych klientów, jeśli numer zlecenia nie występuje w danych źródłowych.
    df_zlec = None
    df["Sales Value"] = np.nan
    df["Data faktury"] = pd.NaT
    df["Źródło Sales Value"] = ""
    ...
    if uf_zlec:
        ...
        if df_zlec is not None:
            ...
            if nzp_z and wart_z:
                ...
                zlec["_sales"] = pd.to_numeric(zlec[wart_z], errors="coerce").fillna(0)
                zlec = zlec[zlec["_nzp"].ne("")].copy()

                # Data faktury: bierzemy najwcześniejszą niepustą datę z grupy
                # tylko do okresu raportowego. Sama sprzedaż jest sumą wszystkich linii.
                if data_z:
                    zlec["_date"] = pd.to_datetime(zlec[data_z], errors="coerce")
                    grp_z = (
                        zlec.groupby("_nzp", dropna=False)
                        .agg(_sv_z=("_sales", "sum"), _dv_z=("_date", "min"))
                        .reset_index()
                        .rename(columns={"_nzp": "_znzp"})
                    )
                else:
                    grp_z = (
                        zlec.groupby("_nzp", dropna=False)
                        .agg(_sv_z=("_sales", "sum"))
                        .reset_index()
                        .rename(columns={"_nzp": "_znzp"})
                    )
                    grp_z["_dv_z"] = pd.NaT
                ...
                df.loc[mask_z, "Źródło Sales Value"] = "zlec+faktury: suma NZP incl. korekty"
                df.drop(columns=["_znzp", "_sv_z", "_dv_z"], errors="ignore", inplace=True)
            else:
                warns.append("Plik Zlecenia+FV: brak kolumn Numer zlecenia produkcyjnego / Wartosc w linii FV netto.")

    # Fallback z Faktury linie jest celowo konserwatywny:
    # przyjmujemy tylko jednoznaczne dopasowanie fragmentu zamówienia do dokładnie jednej linii.
    # Nie ma już przeliczania proporcjonalnego, bo ono mogło tworzyć fikcyjne Sales Value
    # dla FD Pharma, gdy brakowało właściwego zlecenia w pliku zlec+faktury.
        ...
        if df_fry is not None:
            ...
            if nl_c and wl_c:
                ...
                for idx in df[missing_mask].index:
                    ...
                    # Jeśli mamy ilości po obu stronach, wymagamy zgodności.
                    ...
                    df.at[idx, "Źródło Sales Value"] = "faktury linie: exact unique"
                    if dl_c:
    ...
    # ── 9b. COLUMN CLEANUP ───────────────────────────────────────────────────
    # Kolumna "check sprzedaż" została usunięta z pliku docelowego.
    df.drop(columns=["check sprzedaż"], errors="ignore", inplace=True)
    ...
    TAIL = ["Total DL", "Total Materials", "Sales Value",
            "Data faktury", "Miesiąc faktury", "Źródło Sales Value", "TPM", "TPM%", "CM", "CM%"]
