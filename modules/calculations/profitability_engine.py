...
def build_profitability(
    ...
    # -- Column: Nesting (Tools[Die cut] → Tools[Ilość użytków wykrojnika / Nesting]) --
    _tools_nesting_map: dict[str, float] = {}
    if df_post_tools is not None:
        _tl_nm = _fcol(df_post_tools, "Nazwa narzędzia")
        _tl_ne = _fcol(df_post_tools, "Ilość użytków wykrojnika / Nesting",
                       "Ilość użytków wykrojnika/Nesting")
        if _tl_nm and _tl_ne:
            for _, _r in df_post_tools.iterrows():
                _nm_v = str(_r.get(_tl_nm, "")).strip()
                _ne_v = _sn(_r.get(_tl_ne, 0))
                if _nm_v:
                    _tools_nesting_map[_nm_v] = _ne_v

    if _tools_nesting_map:
        df["Nesting"] = (
            df["Die cut"].astype(str).str.strip()
            .map(_tools_nesting_map)
        )
    else:
        df["Nesting"] = _np2.nan
    df["Nesting"] = pd.to_numeric(df["Nesting"], errors="coerce")

    # -- Column: koszt gilotyny -----------------------------------------------
    # Formula (mirrors prekalk_engine.py):
    #   koszt_gilotyny = gilotyna_rate * (ark / liczba_cięć) / gilotyna_wydajnosc
    # where: ark = Zamawiana_ilość_z_FV / Nesting
    ...
    for _nm_wy, _wr_wy in _wy_map.items():
        if "gilotyna" in str(_nm_wy).lower():
            ...
            break
    # Fallback rate from rates dict if not found in wydajnosc
    if _gilotyna_rate == 0:
        _gilotyna_rate = rates.get("Gilotyna", 65.69)

    _qty_fv_arr = pd.to_numeric(
        df.get(qty_fv_base_col, pd.Series(0, index=df.index)), errors="coerce"
    ).fillna(0).values
    _nesting_arr = df["Nesting"].fillna(0).values
    _lc_arr = df["liczba cięć"].fillna(0).values
