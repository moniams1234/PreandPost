...
def build_profitability(
    ...
    # Sales Value must come only from a clearly matched invoice row.
    # The app must NOT invent or proportionally calculate Sales Value when no match exists.
    df_zlec = None
    ...
    if uf_zlec:
        ...
        if df_zlec is not None:
            ...
            if nzp_z and wart_z:
                ...
                if data_z:
                    zlec["_date"] = pd.to_datetime(zlec[data_z], errors="coerce")
                else:
                    zlec["_date"] = pd.NaT
                if ilosc_z:
                    zlec["_qty"] = pd.to_numeric(zlec[ilosc_z], errors="coerce")
                else:
                    zlec["_qty"] = np.nan

                # Prepare maps by production order.
                ...
                def _match_sales_from_zlec(row):
                    ...
                    if not zp or zp not in groups_z:
                        return pd.Series([np.nan, pd.NaT, "brak dopasowania"])

                    g = groups_z[zp].copy()

                    # Case 1: only one positive invoice row (plus optional corrections/negative values).
                    pos_rows = g[g["_sales"].fillna(0) >= 0]
                    neg_rows = g[g["_sales"].fillna(0) < 0]

                    if len(pos_rows) == 1:
                        r = pos_rows.iloc[0]
                        total_sales = g["_sales"].sum(skipna=True)
                        return pd.Series([total_sales, r["_date"], "zlec+FV: jednoznaczne NZP"])

                    # Case 2: multiple invoice rows. Match by quantity.
                    if qty_fv_base_col and "_qty" in g.columns:
                        ...
                        if pd.notna(q):
                            gm = g[np.isclose(g["_qty"].fillna(-999999).astype(float), q, atol=0.01)]
                            if len(gm) == 1:
                                r = gm.iloc[0]

                                # Add all negative corrections for the same production order.
                                negative_sum = g.loc[g["_sales"].fillna(0) < 0, "_sales"].sum(skipna=True)
                                total_sales = float(r["_sales"]) + float(negative_sum)

                                return pd.Series([total_sales, r["_date"], "zlec+FV: NZP + ilość FV"])

                    # Multiple invoice rows but no unique quantity match.
                    # Do not sum, do not guess, leave Sales Value empty/zero.
                    return pd.Series([np.nan, pd.NaT, "brak jednoznacznego dopasowania"])

                matched_sales = df.apply(_match_sales_from_zlec, axis=1)
                matched_sales.columns = ["_matched_sales", "_matched_date", "_matched_source"]

                mask_sales = matched_sales["_matched_sales"].notna()
                df.loc[mask_sales, "Sales Value"] = matched_sales.loc[mask_sales, "_matched_sales"].values
                df.loc[mask_sales, "Data faktury"] = matched_sales.loc[mask_sales, "_matched_date"].values
                df["_matched_source"] = matched_sales["_matched_source"].values

    # Optional fallback from invoice lines:
    # Restore previous correct proportional calculation, but ONLY when a matching
    # invoice line is found in "faktury linie". If not found, Sales Value remains NaN
    # and later becomes 0 — no guessing.
        ...
        if df_fry is not None:
            ...
            if nl_c and wl_c:
                ...
                for idx in df[missing_mask].index:
                    ...
                    qty_val = sn(df.at[idx, qty_fv_base_col]) if qty_fv_base_col else sn(df.at[idx, qty_c] if qty_c else 0)
                    ...
                    matched = [i for i, s in enumerate(fry_names) if frag_lower in s]

                    # If production order/order line does not exist in invoice lines,
                    # do not assign Sales Value.
                    if not matched:
                        continue

                    # Previous accepted behaviour: take first matched invoice line
                    # and calculate proportionally by invoice line quantity.
                    ...
                    if ilosc_fry > 0 and qty_val > 0:
                        ...
                        # Only if the line exists but quantity is unavailable.
                        sv = wart_fry
