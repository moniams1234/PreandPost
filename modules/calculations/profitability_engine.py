...
def build_profitability(
    ...
    # Expected logic:
    # 1) Sales Value can be assigned only from "zlecenia + faktury" or "faktury linie".
    # 2) If there is one invoice row for the production order -> take its net value.
    # 3) If there are multiple invoice rows for the production order -> match by FV quantity.
    # 4) If production order is not found / not uniquely matched in zlec+faktury,
    #    use proportional calculation from faktury linie, but only when an invoice
    #    line containing the order fragment / Lewy 10 exists.
    # 5) If no match exists in either source -> Sales Value remains 0. No guessing.
    df_zlec = None
        ...
        if df_zlec is not None:
            ...
            if nzp_z and wart_z:
                ...
                def _match_sales_from_zlec(row):
                    ...
                    # One row for this production order => direct match.
                    if len(g_valid) == 1:
                        r = g_valid.iloc[0]
                        return pd.Series([r["_sales"], r["_date"], "zlec+FV: NZP"])

                    # Multiple rows => match by quantity from FV.
                    if qty_fv_base_col:
                        ...
                        if pd.notna(q):
                            gm = g_valid[np.isclose(g_valid["_qty"].fillna(-999999).astype(float), q, atol=0.01)]
                            if len(gm) >= 1:
                                r = gm.iloc[0]
                                return pd.Series([r["_sales"], r["_date"], "zlec+FV: NZP + ilość FV"])

                    # No unique quantity match: do not sum, let faktury linie fallback try.
                ...
                df.loc[:, "_matched_source"] = matched_sales["_matched_source"].values

    # ── 5b. faktury linie — proportional fallback only if line exists ─────────
        ...
        if df_fry is not None:
            ...
            if nl_c and wl_c:
                ...
                for idx in df[missing_mask].index:
                    ...
                    lewy_val = str(df.at[idx, "Lewy 10"] if "Lewy 10" in df.columns else "").strip()

                    qty_val = sn(df.at[idx, qty_fv_base_col]) if qty_fv_base_col else sn(df.at[idx, qty_c] if qty_c else 0)

                    # Match by order fragment from Zamówienie; if unavailable, use Lewy 10.
                    frag_candidates = []
                    frag = extract_fry_fragment(zam_val)
                    if frag:
                        frag_candidates.append(frag)
                    if lewy_val:
                        frag_candidates.append(lewy_val)

                    matched = []
                    for frag_one in frag_candidates:
                        fl = str(frag_one).strip().lower()
                        if not fl:
                            continue
                        matched = [i for i, s in enumerate(fry_names) if fl in s]
                        if matched:
                            break

                    # If neither Zlecenie nor Zamówienie/Lewy10 exists in invoice lines,
                    # do not assign Sales Value.
                    if not matched:
                        continue

                    # Previous correct behaviour: first matched line + proportional quantity.
    ...
    # If not found in either source, Sales Value becomes 0.
