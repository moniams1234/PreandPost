...
def build_profitability(
    ...
    # Only exact and unique match is allowed. No proportional calculation.
        ...
        if df_fry is not None:
            ...
            if nl_c and wl_c:
                ...
                for idx in df[missing_mask].index:
                    ...
                    matched = [i for i, s in enumerate(fry_names) if frag.lower() in s]

                    # Only unique match accepted.
                    if len(matched) != 1:
                        continue

                    found_row = df_fry.iloc[matched[0]]

                    # If quantity column exists on both sides, require equality.
                    if il_c and qty_fv_base_col:
                        q1 = sn(df.at[idx, qty_fv_base_col])
                        q2 = sn(found_row[il_c])
                        if not np.isclose(q1, q2, atol=0.01):
                            continue

                    df.at[idx, "Sales Value"] = sn(found_row[wl_c])
                    df.at[idx, "_matched_source"] = "faktury linie: exact unique"
                    if dl_c:
