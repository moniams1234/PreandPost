...
def build_profitability(
    ...
    # Safe result = 0 if any required input is missing.
    ...
    # Fallbacks if Wydajność table does not contain rate / efficiency.
    if _gilotyna_rate == 0:
        _gilotyna_rate = _sn(rates.get("Gilotyna", rates.get("gilotyna", 65.69)))
    if _gilotyna_wy == 0:
        # Do not leave koszt gilotyny empty only because efficiency is missing.
        # Use 1.0 as technical fallback, so the user still sees a calculated cost.
        _gilotyna_wy = 1.0
    ...
    # Diagnostic columns help explain why koszt gilotyny is zero.
    df["gilotyna_rate"] = _gilotyna_rate
    df["gilotyna_wydajność"] = _gilotyna_wy
    df["gilotyna_check"] = _np2.where(
        _valid_gilotyna,
        "OK",
        _np2.where(_nesting_arr <= 0, "brak Nesting",
        _np2.where(_lc_arr <= 0, "brak liczba cięć",
        _np2.where(_qty_fv_arr <= 0, "brak ilości FV",
        _np2.where(_gilotyna_rate <= 0, "brak stawki gilotyny", "brak wydajności gilotyny"))))
    )
