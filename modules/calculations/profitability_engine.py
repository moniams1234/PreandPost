...
def build_profitability(
    ...
    # Sales Value logic:
    # - only from zlecenia + faktury or faktury linie
    # - zlec+faktury: match by production order; if many positives, match by FV quantity
    # - all negative invoice rows for the same production order are added as corrections
    # - faktury linie fallback: proportional calculation only if invoice line is found
    # - no match in any source => Sales Value = 0
    df_zlec = None
    ...
    if uf_zlec:
        ...
        if df_zlec is not None:
            ...
            if nzp_z and wart_z:
                ...
                def _sales_plus_negative_corrections(pos_row, group_all):
                ...
                def _match_sales_from_zlec(row):
                    ...
                    positive_rows = g_valid[g_valid["_sales"] >= 0].copy()
                    if positive_rows.empty:
                        return pd.Series([np.nan, pd.NaT, "zlec+FV: tylko korekty ujemne"])

                    if len(positive_rows) == 1:
                        r = positive_rows.iloc[0]
                        total_sales = _sales_plus_negative_corrections(r, g_valid)
                        return pd.Series([total_sales, r["_date"], "zlec+FV: NZP + korekty ujemne"])
                    ...
                    if qty_fv_base_col:
                        ...
                        if pd.notna(q):
                            gm = positive_rows[np.isclose(
                                positive_rows["_qty"].fillna(-999999).astype(float),
                                q,
                                atol=0.01,
                            )]
                            if len(gm) >= 1:
                                r = gm.iloc[0]
                                total_sales = _sales_plus_negative_corrections(r, g_valid)
                                return pd.Series([total_sales, r["_date"], "zlec+FV: NZP + ilość FV + korekty ujemne"])
