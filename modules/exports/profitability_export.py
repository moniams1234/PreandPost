...
def build_xlsx(
    ...
    df_orders_extra = result.get("df_orders")
    df_tektura_extra = result.get("df_tektura")
    df_material_extra = result.get("df_material")
    ...
    # Sheet contains rows from Profitability where Sales Value was returned
    # from Faktury-linie / Linie faktury source.
    ...
    # ── usługa na surowcu ────────────────────────────────────────────────────
    if df_material_extra is not None:
        ws_mat_srv = wb.create_sheet("usługa na surowcu")
        xlsx_write_sheet(
            ws_mat_srv,
            df_material_extra,
            curr_cols={c for c in df_material_extra.columns if is_currency_col(c)},
        )

    # ── orders ────────────────────────────────────────────────────────────────
    if df_orders_extra is not None:
        ws_orders = wb.create_sheet("orders")
        xlsx_write_sheet(
            ws_orders,
            df_orders_extra,
            curr_cols={c for c in df_orders_extra.columns if is_currency_col(c)},
        )

    # ── tektura ───────────────────────────────────────────────────────────────
    if df_tektura_extra is not None:
        ws_tektura = wb.create_sheet("tektura")
        xlsx_write_sheet(
            ws_tektura,
            df_tektura_extra,
            curr_cols={c for c in df_tektura_extra.columns if is_currency_col(c)},
        )
