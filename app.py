...
_SS_DEFAULTS: dict[str, Any] = {
    ...
    # PostKalkulacja supplementary tables (Tools + Wydajnosc)
    "post_tools_df":      None,
    "post_wydajnosc_df":  None,
    # PreKalkulacja
    "pk_result":          None,
    "pk_tools_df":        None,
...
def recalculate_profitability(show_success: bool = True) -> bool:
    ...
    # Read supplementary PostKalkulacja files
    ...
    from modules.readers.tools_reader import read_prekalk_tools, empty_tools_df
    from modules.readers.wydajnosc_reader import read_prekalk_wydajnosc, empty_wydajnosc_df

    _df_mat  = read_prekalk_material_service(current_file("post_material"))
    _df_tek  = read_prekalk_tektura(current_file("post_tektura"))
    _df_ord  = read_prekalk_orders(current_file("post_orders"))

    # Default Tools/Wydajność must be available without visiting their tabs.
    # If user saved/recalculated edited/uploaded data, use session_state.
    # Otherwise use bundled defaults.
    _df_tl = st.session_state.get("post_tools_df")
    if _df_tl is None or getattr(_df_tl, "empty", True):
        _loaded_tools = read_prekalk_tools(None)
        _df_tl = _loaded_tools if _loaded_tools is not None else empty_tools_df()
        st.session_state["post_tools_df"] = _df_tl

    _df_wy = st.session_state.get("post_wydajnosc_df")
    if _df_wy is None or getattr(_df_wy, "empty", True):
        _loaded_wy = read_prekalk_wydajnosc(None)
        _df_wy = _loaded_wy if _loaded_wy is not None else empty_wydajnosc_df()
        st.session_state["post_wydajnosc_df"] = _df_wy
    ...
    with st.spinner("Przeliczanie profitability…"):
        result, warns = build_profitability(
            ...
            df_post_material=_df_mat,
            df_post_tektura=_df_tek,
            df_post_orders=_df_ord,
            df_post_tools=_df_tl,
            df_post_wydajnosc=_df_wy,
        )
    if result:
...
def clear_all_inputs() -> None:
    POST_UPLOAD_KEYS = [
        ...
        "post_material", "post_orders", "post_tektura",
        "post_tools_file", "post_wydajnosc_file",
    ]
    PK_UPLOAD_KEYS = [
    ...
    for key in ["result", "pk_result", "pk_tools_df", "pk_wydajnosc_df",
                "post_tools_df", "post_wydajnosc_df", "settings_changed"]:
...
if active_module == "PostKalkulacja":
    ...
    if page == POST_PAGES[0]:
        ...
        from modules.ui.post_tools_page import render as render_post_tools
        try:
            render_post_tools(recalculate_profitability)
        except TypeError:
            # Backward compatibility if an older post_tools_page.py is still deployed.
            render_post_tools()

    elif page == POST_PAGES[6]:
        from modules.ui.post_wydajnosc_page import render as render_post_wy
        try:
            render_post_wy(recalculate_profitability)
        except TypeError:
            # Backward compatibility if an older post_wydajnosc_page.py is still deployed.
            render_post_wy()
